...
public class BookingController extends HttpServlet {
    ...
    private void returnPrintWriter(List<Slot> slotList, PrintWriter printWriter) {
        for (int i = 0; i < slotList.size(); i++) {
            printWriter.println("<input type=\"radio\" class=\"btn-check\" name=\"options-outlined\" id=\"success-outlined" + i + "\" value=\"" + slotList.get(i).getSlotStart()+ "\" onchange=\"change_Book(this,'buttonSubmit','selService','dateService')\">\n"
                    + "                                <label id=\"slot" + i + "\" class=\"btn btn-outline-primary\" for=\"success-outlined" + i + "\">" + slotList.get(i).getSlotStart() + "</label>");
        }
    }
